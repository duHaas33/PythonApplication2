...
class R10Device(BluetoothDeviceBase):
    ...
    def __process_message(self, data: bytearray) -> None:
        ...
        if calculated_crc != supplied_crc:
            ...
            if hex_msg.upper().startswith("A013"):
                ...
                print(f'XXXXXXXXXXXXXXXXXXXXX {hex_msg.upper()}')
                ack_body.extend(message_data[2:4])
                ack_body.extend(bytearray.fromhex("00000000000000"))
                proto = WrapperProto()
                print(f'222222222222222222222222222222222222222222222222 {hex_msg.upper()}')
                print(f'YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY {ack_body}')

                proto.ParseFromString(bytes(message_data[16:]))
                print(f'3333333333333333333333333333333333333333333333333333 {hex_msg.upper()}')

                #print(f'(__process_message) proto: {proto}')

                print(f'44444444444444444444444444444444444444444444444444444 {hex_msg.upper()}')
    ...
    def __handle_protobuf_response(self, request: Message):
        ...
        if request.HasField('service'):
            ...
            print('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxYYYYYYYYYYYYYYYYYYYYYYYYYYYYY')
