...
class BluetoothDeviceService(QObject):
    ...
    def subscribe_to_notifications(self) -> None:
        print("We in here but can't find it")
        self.status_update.emit('Subscribing...', "Approach R10")
        for uuid in self._characteristic_uuids:
            msg = f"Subscribing to notifications for service: {self._service_uuid.toString()} characteristic: {uuid.toString()} on Approach R10"
            logging.debug(msg)
            print(msg)
            characteristic = self._service.characteristic(uuid)
            if not characteristic.isValid():
                msg = f"Couldn't find characteristic {uuid.toString()} on Approach R10."
                logging.debug(msg)
                self.error.emit(msg)
                return
            ...
            if not descriptor.isValid():
                msg = f"Characteristic descriptor is invalid for {uuid.toString()} on Approach R10."
                logging.debug(msg)
                self.error.emit(msg)
                return
            ...
            print(f'Subscribed to notifications for {uuid.toString()} on Approach R10')
